 ...
 *
 * so accept either. Invalid values are treated as an explicitly empty allowlist
 * rather than silently inheriting write-capable tools. Omitted `tools` is kept
 * distinct by `toolsSpecified` so full-capability agents can opt in explicitly.
 */
...
function loadAgentsFromDir(dir: string, source: "user" | "project"): AgentConfig[] {
	...
	for (const entry of entries) {
		...
		agents.push({
			...
			toolsSpecified: Object.prototype.hasOwnProperty.call(frontmatter, "tools"),
...
export function formatAgentList(agents: AgentConfig[], maxItems: number): { text: string; remaining: number } {
	...
	return {
		text: listed.map(formatAgentLine).join("; "),
		remaining,
	};
}
